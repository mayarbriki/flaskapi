...
def stats():
    ...
    avg_overall = round(float(df["overall_rating_original"].mean()), 2) if _has("overall_rating_original") else None
    avg_value_million = round(float(df["value_euro_original"].mean())/1e6, 2) if _has("value_euro_original") else None
    most_common_nat = df["nationality_original"].mode()[0] if _has("nationality_original") else None

    return jsonify({
        "total_players": int(len(df)),
        "avg_overall": avg_overall,
        "avg_value_million": avg_value_million,
        "most_common_nationality": most_common_nat
    })
...
def top_players():
    """
    Classement des meilleurs joueurs selon un critère (overall_rating, value_euro, potential...).
    Exemple : /top/players?metric=overall_rating&limit=10
    """
    metric = request.args.get("metric", "overall_rating")
    limit = int(request.args.get("limit", 10))

    if not _has(metric) and not _has(metric + "_original"):
        return jsonify({"error": f"Colonne inconnue: {metric}"}), 400

    col = metric + "_original" if _has(metric + "_original") else metric
    out_cols = [c for c in ["name", "full_name", "nationality_original", "positions_original", col] if _has(c)]

    top = df.nlargest(limit, col)[out_cols].copy()
    top.rename(columns={
        "nationality_original": "nationality",
...
@app.route("/players/position/<pos>", methods=["GET"])
def players_by_position(pos):
    """Liste quelques joueurs pour un poste donné (ex: ST, GK, CM)."""
    col = "positions_original" if _has("positions_original") else "positions"
    subset = df[df[col].astype(str).str.contains(pos, case=False, na=False)]

    out_cols = [c for c in ["name", "full_name", "nationality_original", col, "overall_rating_original"] if _has(c)]
    result = subset[out_cols].head(20).copy()
...
@app.route("/top/nations", methods=["GET"])
def top_nations():
    """Classement des nations par moyenne d'overall."""
    nat_col = "nationality_original" if _has("nationality_original") else "nationality"
    overall_col = "overall_rating_original" if _has("overall_rating_original") else "overall_rating"

    grp = (
        df[[nat_col, overall_col]]
        .dropna()
        .groupby(nat_col, as_index=False)[overall_col]
        .mean()
        .rename(columns={nat_col: "nationality", overall_col: "avg_overall"})
