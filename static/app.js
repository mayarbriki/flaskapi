...
function showStats(data) {
  extraOutput.innerHTML = `
    <table>
      <tr><th>Total Players</th><td>${data.total_players}</td></tr>
      <tr><th>Average Overall</th><td>${data.avg_overall}</td></tr>
      <tr><th>Average Value (M€)</th><td>${data.avg_value_million}</td></tr>
      <tr><th>Most Common Nationality</th><td>${data.most_common_nationality}</td></tr>
    </table>
  `;
}
...
function showPlayers(players) {
  if (!Array.isArray(players) || players.length === 0) {
    extraOutput.innerHTML = "<p>No players found.</p>";
    return;
  }
...
function showNations(nations) {
  if (!Array.isArray(nations) || nations.length === 0) {
    extraOutput.innerHTML = "<p>No nations found.</p>";
    return;
  }

  let rows = nations.map(n => `
    <tr>
      <td>${n.nationality}</td>
      <td>${n.avg_overall.toFixed(2)}</td>
    </tr>
  `).join("");
...
document.getElementById("btn-top-players").addEventListener("click", async () => {
  const res = await fetch("/top/players?metric=overall_rating&limit=10");
  const data = await res.json();
  showPlayers(data);
});

document.getElementById("btn-top-nations").addEventListener("click", async () => {
  const res = await fetch("/top/nations?limit=10");
  const data = await res.json();
  showNations(data);
});
